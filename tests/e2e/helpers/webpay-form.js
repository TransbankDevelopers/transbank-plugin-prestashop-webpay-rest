...
export async function fillCardAndAuthenticate(page) {
  await page.locator(S.tarjetasBtn).waitFor({ state: 'visible', timeout: 15_000 });
  await page.locator(S.tarjetasBtn).click();

  await page.locator(S.cardNumber).waitFor({ state: 'visible', timeout: 15_000 });
  ...
  await page.locator(S.cardExpiry).waitFor({ state: 'visible', timeout: 15_000 });
  await page.locator(S.cardExpiry).fill(TEST_CARD.expiry);

  await page.locator(S.cardCvv).waitFor({ state: 'visible', timeout: 10_000 });
  await page.locator(S.cardCvv).fill(TEST_CARD.cvv);

  await page.locator(S.payButton).click();

  await page.locator(S.bankRut).waitFor({ state: 'visible', timeout: 30_000 });
  ...
  await page.locator(S.resultVci).waitFor({ state: 'visible', timeout: 30_000 });
}
...
export async function continueToCommerce(page) {
  await expect(page.locator(S.resultVci)).toHaveValue('TSY', { timeout: 30_000 });
